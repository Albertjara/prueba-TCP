...
def parse_extended_eb_fields(additional_value):
    ...
    Asume el formato propietario: Longitud de Bloque (2 bytes) + ID (2 bytes) + Valor (N bytes).
    """
    sub_data = additional_value
    
    # El primer campo parece ser un encabezado fijo (000c00) que rompe la sincronía. Lo saltamos.
    # El resto de los campos siguen la estructura Length(2) + ID(2) + Value(N).
    sub_current_byte = 3 
    ...
    while sub_current_byte < len(sub_data):
        ...
        if sub_current_byte + 2 > len(sub_data):
            print(f"      [ADVERTENCIA] Faltan bytes para leer la Longitud del Bloque (pos: {sub_current_byte}).")
            break
            
        sub_length_block_raw = sub_data[sub_current_byte:sub_current_byte+2]
        sub_length_block = int.from_bytes(sub_length_block_raw, 'big')

        # 2. Leer ID del Bloque (2 bytes)
        id_start = sub_current_byte + 2
        if id_start + 2 > len(sub_data):
            print(f"      [ADVERTENCIA] Faltan bytes para leer el ID (pos: {id_start}).")
            break

        sub_id_raw = sub_data[id_start:id_start+2]
        sub_id = int.from_bytes(sub_id_raw, 'big')
        
        # 3. Calcular Longitud del Valor (N = Longitud Total - 2 bytes de ID)
        value_start = id_start + 2
        value_length = sub_length_block - 2 
        value_end = value_start + value_length

        if value_end > len(sub_data):
            print(f"      [ERROR DE PARSING] Campo {hex(sub_id)} longitud de valor ({value_length}) inválida. Deteniendo.")
            break
            
        sub_value_bytes = sub_data[value_start:value_end]
        
        print(f"      - ID Sub-campo: {hex(sub_id)}, Longitud Bloque: {sub_length_block}, Longitud Valor: {value_length}, Valor (HEX): {sub_value_bytes.hex()}")

        if sub_id == 0x00B2:
            # 0x00B2 (Número ICCID) - 10 bytes BCD (si value_length es 10)
            iccid = sub_value_bytes.hex()
            print(f"        - ICCID: {iccid}")

        elif sub_id == 0x002D:
            # 0x002D (Valor de voltaje) - 2 bytes de valor (4 bytes de bloque)
            if value_length == 2:
            ...
            # 0x00A8 (Porcentaje de batería) - 1 byte de valor (3 bytes de bloque)
            if value_length == 1:
                percentage = sub_value_bytes[0]
                print(f"        - **Porcentaje de Batería**: {percentage} %")

        elif sub_id == 0x00D5:
            # 0x00D5 (Número IMEI del dispositivo) - 15 bytes de valor (17 bytes de bloque)
            if value_length == 15:
                try:
            ...
            try:
                ...
                # El formato interno es DC:62:79:54:09:DE,-47,2A:47:3D:45:E3:BD,-54...
...
def handle_client(conn, addr):
    ...
    try:
        while True:
            ...
            print(f"  --> Número de Serie: {message_serial_number}")
            ...
            if message_id == 0x0200: 
                ...
                if len(message_body) < 28:
                    ...
                    # Decodificación de información básica (Latitud, Longitud, Hora)
                    ...
                    if len(message_body) > additional_info_start:
                        ...
                        while current_byte < len(message_body):
                            ...
                            elif additional_id == 0x33: # Modo de dispositivo (Restaurado)
                                device_mode = int.from_bytes(additional_value, 'big')
                                mode_desc = "Ultra-larga duración" if device_mode == 0x01 else "Desconocido/Normal"
                                print(f"    - Modo de dispositivo: {device_mode} ({mode_desc})")
                            elif additional_id == 0xeb:
                                print("    - INICIO DE SUB-TRAMA EXTENDIDA (0xEB) - ID 2 bytes, Longitud 2 bytes")
            ...
            if response_message_id:
                # Construcción de la respuesta 0x8001
